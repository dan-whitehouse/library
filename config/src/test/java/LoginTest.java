...
public class LoginTest {
	...
	private static void postVendor(ConfigRequest request) {
		...
		ConfigPath path = new ConfigPathBuilder(ServicePath.VENDORS).build();
		ConfigResponse<Vendor> response = request.postVendor(path, vendor);
		System.out.println(response.getData());
		System.out.println(response.getResponseStatus());
	}

	private static void putVendor(ConfigRequest request) {
		Vendor vendor = new Vendor();
		vendor.setId("PostVendor1");
		vendor.setEnabled(false);

		ConfigPath path = new ConfigPathBuilder(ServicePath.VENDORS).build();
		ConfigResponse<Vendor> response = request.putVendor(path, vendor);
		System.out.println(response.getData());
		System.out.println(response.getResponseStatus());
	}

	private static void deleteVendor(ConfigRequest request) {
		ConfigPath path = new ConfigPathBuilder(ServicePath.VENDOR).id("PostVendor1").build();
		ConfigResponse response = request.deleteVendor(path);
		System.out.println(response.getResponseStatus());
	}
}
